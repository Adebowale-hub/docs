...
<Tabs>
  ...
  <Tab title="Custom ID">
    ...
    console.log(doc.id); // "user_alice_2024"
    ```
...
{" "}
<Tab title="Dart">
  ```dart final collections = await db.listCollections(); print(collections); //
  ["users", "posts", "products"] ```
</Tab>

{" "}
<Tab title="Go">
  ```go collections, err := db.ListCollections(ctx) // ["users", "posts",
  "products"] ```
</Tab>
...
{" "}
<Tab title="Dart">```dart await db.deleteCollection('old_data'); ```</Tab>

{" "}
<Tab title="Go">```go err := db.DeleteCollection(ctx, "old_data") ```</Tab>
